...
void infixToPostfix(char expr[], char result[]) {//중위수식을 후위수식으로
	...
	for (int i = 0; i < n; i++) {
		...
		if (isalnum(c)) //피연산자면 N스택에 넣기
			...
			if (!isEmpty(&NS) && NS.top == 0) {//후위수식만드는 함수를 재호출 하기 위해 1자리 수식은 그냥 작동
				result[idx++] = pop(&NS);
			}
			else if (!isEmpty(&NS)){
				char temp[N];
				...
				char temp_result[N];
				infixToPostfix(temp, temp_result); //후위 수식으로 바꾸기

				for (int j = 0; temp_result[j] != '\0'; j++) {
					result[idx++] = temp_result[j];  // result 뒤에 붙이기 (숫자 하나를 식으로 바꿨으니 아마 그냥 붙여도 되지않을까...)
				}
			}
	...
	if (!isEmpty(&NS) && NS.top == 0) {//후위수식만드는 함수를 재호출 하기 위해 1자리 수식은 그냥 작동
		...
		while (!isEmpty(&NS)) {//스택에서 숫자 다 뽑기
			temp[temp_idx++] = pop(&NS);
			for (int i = 0; i < degree; i++) {//차수 만큼 10 곱하기
				temp[temp_idx++] = '*';
				temp[temp_idx++] = '2';
				temp[temp_idx++] = '*';
				temp[temp_idx++] = '5';
